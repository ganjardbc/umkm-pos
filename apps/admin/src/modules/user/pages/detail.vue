<template>
  ...
  <AssignOutletModal
    ...
    :merchant-id="userDetail?.merchant_id"
...
import { ASSIGN } from '@/modules/role/services/rbac.ts';
...
const isCanAssign = computed(() => isHasPermission(ASSIGN));
...
const assignRole = async (roleId: string, outletId: string) => {
  try {
    showLoading();
    const payload = {
      role_id: roleId,
    ...
    const response = await assignRoleToUser(userID.value, payload);
...
const revokeRole = async (roleId: string, outletId: string) => {
  try {
    showLoading();
    const payload = {
      role_id: roleId,
    ...
    const response = await revokeRoleFromUser(userID.value, payload);
