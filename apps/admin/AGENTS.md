...
- Module API services call `/api/v1/admin/*`; do not point admin features at tenant endpoints (those are scoped to the caller's own merchant).
...
- There is no active outlet: permissions are the union across every role the admin holds.
- `isHasPermission()` always includes `dashboard.view` by default; this can affect guard/debug expectations.
