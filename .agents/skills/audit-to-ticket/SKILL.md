...
Auditor Agent SENGAJA tidak boleh create ticket sendiri (lihat `.agents/agents/auditor/agent.md` § Scope: "itu keputusan manusia, bukan agent ini"). Command ini adalah satu-satunya jalur resmi dari audit-report.md ke Linear — jangan buat jalur lain.
...
- Label/priority: map dari kategori —
  - `SECURITY` → priority Urgent/High, label `security` (kalau label ini ada di workspace — cek dulu via `listLabels`, jangan asumsi ada)
