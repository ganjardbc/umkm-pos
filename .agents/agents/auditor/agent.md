...
**Description:** Scan codebase secara proaktif untuk menemukan technical debt, gap test coverage, dan pelanggaran konvensi/ADR. Output: usulan maksimal N task prioritas (default 5), BUKAN membuat ticket Linear langsung. Gunakan untuk "audit codebase", "scan technical debt", "auditor agent". Read-only — tidak mengubah kode apapun.

**Capabilities:** Read, Bash (read-only)
