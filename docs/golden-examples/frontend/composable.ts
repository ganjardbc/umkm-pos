/**
 * GOLDEN EXAMPLE — Frontend Composable
 *
 * Source: apps/web/src/modules/shift/composables/useShift.ts
 *
 * Rules illustrated (POLA STRUKTURAL — lihat catatan di bawah):
 * - Singleton reactive state di luar fungsi composable agar shared antar component instance
 * - Computed guards (isShiftClosed, isUserOwner, isUserInShift) derived dari state — tidak disimpan terpisah
 * - Async action wrappers: try/finally untuk loading state, propagate error ke caller
 * - Return shape yang eksplisit: state, computed, actions — tidak return seluruh internal scope
 * - Import API calls dari services/api (bukan axios langsung)
 *
 * ⚠️  CATATAN: File ini adalah contoh POLA STRUKTURAL, TIDAK untuk ditiru type safety-nya.
 *     Banyak `any` type yang seharusnya diganti dengan interface yang proper.
 *     Gunakan file ini sebagai referensi struktur composable, bukan sebagai contoh typing.
 */
...
// Singleton state — dibuat sekali di luar fungsi agar disharing antar component
const shiftState = {
