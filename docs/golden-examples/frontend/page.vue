<!--
  GOLDEN EXAMPLE — Frontend Vue Page

  Source: apps/web/src/modules/product-categories/pages/detail.vue

  Rules illustrated:
  - <script setup lang="ts"> composition API — tidak pakai Options API
  - Gunakan KETIGA services files: services/api.ts, services/constants.ts, services/rbac.ts
  - RBAC gating UI via isHasPermission(UPDATE) dari @/helpers/auth — bukan hardcode role
  - Navigasi gunakan PREFIX_ROUTE_NAME dari constants (bukan hardcode string route name)
  - Error handling via showToast + getErrorMessage dari helpers — konsisten di seluruh modul
  - onMounted untuk fetch data awal
  - State lokal (ref) karena data tidak disharing ke komponen lain — tidak butuh store
-->
