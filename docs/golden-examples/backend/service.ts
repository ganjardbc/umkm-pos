/**
 * GOLDEN EXAMPLE — Backend Service
 *
 * Source: apps/api/src/store-tables/store-tables.service.ts
 *
 * Rules illustrated:
 * - ADR-001: semua query Prisma scope merchant_id via param (bukan dari DTO/body)
 * - ADR-002: created_by + updated_by di semua mutasi untuk audit trail
 * - Private helpers (assertOutlet, ensureUniqueCode) untuk pre-condition validation
 *   — validasi bisnis tidak bocor ke controller
 * - Error types konsisten: NotFoundException, ConflictException, BadRequestException
 * - Outlet cross-check (assertOutlet) mencegah akses data lintas-outlet dalam merchant yang sama
 * - Tidak ada new PrismaClient() — inject via constructor
 */
