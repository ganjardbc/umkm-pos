...
description: >
  ...
  Output: review-notes.md di .caf/tasks/TICKET-ID/.
...
- **Baca:** Semua file + semua artifact di `.caf/tasks/<TICKET-ID>/`
- **Jalankan:** Grep-based security checks
- **Tulis:** Hanya `.caf/tasks/<TICKET-ID>/review-notes.md`
- **Jangan ubah:** Kode, schema, atau artifact lain

## Tools yang Diizinkan

Read (semua), Bash (grep-based checks)

## Input

```
.caf/tasks/<TICKET-ID>/requirements.md
.caf/tasks/<TICKET-ID>/design.md        (jika ada)
.caf/tasks/<TICKET-ID>/verify-report.md
.caf/tasks/<TICKET-ID>/qa-report.md     (jika ada)
...
`.caf/tasks/<TICKET-ID>/review-notes.md`
