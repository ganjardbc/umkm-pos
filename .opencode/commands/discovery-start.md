...
Jalankan peran PM Agent dengan slug sebagai konteks. Sumber kebenaran aturan agent ini adalah
`.opencode/agent/pm.md` — baca file itu dan patuhi isinya. Kalau file itu tidak ada, gunakan
...
Kalau `docs/feature-catalog.md` ada, baca dulu sebagai konteks — cek apakah fitur ini
overlap dengan yang sudah ada (kalau ya, catat di `## Dependency`).

## 4. Penilaian kebutuhan UX Designer

Setelah `prd.md` selesai, nilai dari deskripsi fitur: apakah fitur ini
menyentuh permukaan user — yaitu ada UI/layar baru, atau alur interaksi user yang berubah.

Project ini punya UX Designer Agent (`.opencode/agent/ux-designer.md`), jadi ada dua pilihan:

- **Menyentuh permukaan user** (ada UI/layar baru, atau alur interaksi yang berubah) →
  gunakan agent UX Designer dengan slug yang sama sebagai konteks. UX Designer yang menulis
  `.ai/discovery/{slug}/flow.md`, mengikuti scope dan aturan di
  `.opencode/agent/ux-designer.md`.
- **Tidak menyentuh permukaan user** (mis. perubahan internal, job background, kontrak API
  antar service) → JANGAN panggil UX Designer. PM sendiri yang menulis `flow.md`
...
Sebelum mengaku selesai, konfirmasi bahwa kedua file benar-benar ada di
`.ai/discovery/{slug}/`:
