...
**Command ini READ-MOSTLY. Boleh menulis ke `.ai/tasks/{TICKET-ID}/design.md` saja — JANGAN sentuh kode aplikasi, migration, atau schema apapun.**
...
`.ai/tasks/{TICKET-ID}/requirements.md` dari Planner (jalankan `/plan-ticket` dulu kalau
...
- ADR yang relevan di `docs/decisions/`

## Ikuti Pola Architect Agent

Kalau `.opencode/agent/architect.md` ada, **baca file itu dan patuhi isinya sebagai sumber
...
4. Cek konsistensi dengan ADR yang sudah ada di `docs/decisions/`. Kalau pendekatan yang
...
WAJIB tulis hasil ke `.ai/tasks/{TICKET-ID}/design.md` — jangan cuma tampilkan di chat.
